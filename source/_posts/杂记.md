...
- 两者的区别也类似于 HTML DOM 元素中设置属性，如果通过`dom.style.`的方式进行设置，则不会改变 HTML 的结构，如果通过`dom.setAttribute`这个 API 就直接触发 HTML 结构变化引起 DOM 视图的变化和重新渲染。两者都有可能引起视图变化，后者直接触发视图更新。
