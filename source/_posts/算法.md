...
如果一棵二叉树只有度为0的结点和度为2的结点，并且**度为0的结点在同一层上**，则这颗二叉树就为满二叉树。
...
4、平衡二叉树（左子树右子树高度差不超过1的方式向下递归）

平衡二叉树是一颗空树或它的左右两个子树的高度差的绝对值不超过1，并且左右两个子树都是一颗平衡二叉树。
...
B树是一种**多路搜索树**，它的每个节点可以拥有多于两个孩子节点。M路的B树最多能拥有M个孩子节点。路数越多，树的高度越低。不限制路数，B树就退化成一个有序数组了。

**B树常用于文件系统的索引**，文件系统和数据库的索引都是存在硬盘上的，如果数据量大的话，不一定能一次性加载到内存中（运行时内存）。这时就需要使用**B树多路存储**的威力，可以每次加载B树的一个节点，然后一步步往下找。

在内存中，红黑树确实比B树效率更高，但是当涉及到磁盘操作时，B树就更优了。
...
- 用数组存储：如果父节点数组下标是i，则它的左孩子就是  i * 2 + 1 , 右孩子就是  i * 2 + 2 
...
- 后序遍历：F D B H I G E C A
  ...
  - 即：**先序遍历和后序遍历移除根节点后：**
    ...
    - 先序遍历结果去掉根节点后，前n个就是左子树的先序遍历结果，后m个就是右子树先序遍历结果
    - 后序遍历结果去掉根节点后，前n个就是左子树的先序遍历结果，后m个就是右子树先序遍历结果
...
var averageOfLevels = function(root) {
    const res = [],queue = [root];
    while (queue.length) {
        const len = queue.length;let sum = 0;
        for(let i=0;i<len;i++){
            const curNode = queue.shift();
            sum += curNode.val;
            curNode.left && queue.push(curNode.left);
...
var binaryTreePaths = function(root) {
    if (!root) return [];
    const stack = [root], paths = [''], res = []; //使用栈时：每个栈数据，对应一个路径
    while (stack.length) {
...
var hasPathSum = function(root, targetSum) {
    if(!root) return false;
    const queue = [root] , sumQueue = [0];
    while (queue.length) {
        const len = queue.length;
        for(let i=0;i<len;i++){
            const curNode = queue.shift();
            const curNum = sumQueue.shift();
            if(!curNode.left && !curNode.right && curNode.val+curNum===targetSum){
                return true;
            }
            if(curNode.left){
